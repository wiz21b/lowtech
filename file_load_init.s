;;; The code in this file is expectedd to run once to setup
;;; the IRQ handler. Once that is done, it can be safely removed
;;; from memory.

;;; The RWTS (rdadr16, read16) code is a direct copy from the book :
;;; "Apple ][ ProDOS 1.7 Operating System Source Listing", Apple Computer 1988
;;; It can be found here for example :
;;; http://www.1000bit.it/support/manuali/apple/Apple2ProDOS17Source1988.pdf
...
sector_times:	.repeat CALIBRATION_RUNS
	...
	;; $7FFF to make sure the minimum will be computed later on.
	;; (I must admit it's a signed number 'cos I'm not sure of
	;; what BMI/BPL do and too tired to actually look into that :-) )
...
calibration_loop:
	...
	;; We time the parts in brackets :
	;; - For addr_time :
	;;    ... DATA - loop - [Gap - ADDR] - Gap - DATA - loop ...
	;; - For sector time
	;;    ... DATA - loop - [Gap - ADDR - Gap - DATA] - loop ...
	;; - So the data time will be computer as sector - addr times.
	;;   And on the schema :
	;;    ... DATA - loop - Gap - ADDR - [Gap - DATA] - loop ...

	;; Also, keep in mind that even on perfect disks (the kind
	;; generated by the emulators), the last sector (15)is often
	;; followed by a gap (before reaching the first sector, 0)
	;; that is much longer than usual. So if the read/write head
	;; crosses that gap, the measured times will be noticably
	;; longer (on the order of $200 cycles).

	;; On a real disk, timings can vary much more (even in the data
	;; part which should be precisely the same). But the gap
	;; between sector 0 and 15 seems to be even bigger at times
	;; ($600 cycles !)
	...
	ldx #SLOT_SELECT
	JSR rdadr16
	read_timer2 addr_times	; sector_count will be used as an index
	...
	;; Now that the timings are gathered, we compute their minimum.
	;; We compute the minium because it will be the safest timing
	;; to work with.
...
sum_loop:
	...
	;; CYCLES_FOR_READING_TIMER must be *substracted*
	;; CYCLES_FOR_WRITING_TIMER_CONST must be *added*
	;; Since the former is bigger than the later, we construct
	;; OVERAL_CYCLES_CORRECTION so that it can be substracted.

	OVERAL_CYCLES_CORRECTION = CYCLES_FOR_READING_TIMER - CYCLES_FOR_WRITING_TIMER_CONST

	sub_const_to_16 data_time, OVERAL_CYCLES_CORRECTION
	sub_const_to_16 sector_time, OVERAL_CYCLES_CORRECTION

	;; data_time := sector - addr time (data time is misnamed until here)
...
done_loop:
	...
	;; Cycles that runs from the very start of the IRQ call
	;; to the moment we reset the 6522. It is assumed that we
	;; reset the timer the closest possible to the IRQ "top"
	;; and that the IRQ "top" has a special meaning such as
	;; beginning of a sector, beginning of a PT3 tick, etc.
	;; See the code of IRQ to determine this value.

	CYCLES_FOR_IRQ_ENTRY = 23+35
	...
	sub_const_to_16 disk_irq_handler2::full_sector_time, CYCLES_FOR_IRQ_ENTRY
	...
	sub_const_to_16 disk_irq_handler2::data_time_plus_tolerance, CYCLES_FOR_IRQ_ENTRY
	...
	sub_const_to_16 disk_irq_handler2::twice_full_sector_time, CYCLES_FOR_IRQ_ENTRY
	...
	sub_const_to_16 disk_irq_handler2::full_sector_time_minus_twice_tolerance, CYCLES_FOR_IRQ_ENTRY
...
sector_count:	.byte 0
addr_times:	.repeat ::CALIBRATION_RUNS
	.word 0
	.endrepeat
...
;;; ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	;; activate the MB's so that it is not shadowed
	;; by the mouse. In this case, I assume the
	;; mockingboard is either in slot 4 or 5.
	;; Dunno if it's always like that in the IIc...
	;; FIXME In PT3 player, deater proposes anothe method which
	;; I should use (and which I use nonetheless :-))
...
.endif
	...
	;; Now read whatever ADDR field that comes
	CLC
notyet:
	ldx #SLOT_SELECT
	jsr rdadr16
	bcs notyet		; basic retry :-)

	;; Now we have the ADDR field, we can know the track we're
	;; on. Set up the seek routine with that.
